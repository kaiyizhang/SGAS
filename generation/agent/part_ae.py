...
class PartAEAgent(BaseAgent):
    ...
    def train_model(self, data, args):
        ...
        if self.config.CD_EMD == 'EMD':
            self.loss = self.loss_emd(self.output, self.input)
        if self.config.CD_EMD == 'CD':
    ...
    def val_model(self, data):
        ...
        if self.config.CD_EMD == 'EMD':
            ...
            self.emd[self.config.class_choice]['num'] += B
            self.emd[self.config.class_choice]['value'] += value*B
        if self.config.CD_EMD == 'CD':
